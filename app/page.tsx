...
function readUsers(): Record<string, HabitUser> {
...
function getUser(userId: string): HabitUser | null {
  if (!userId) return null
  return readUsers()[userId] ?? null
}

function saveUser(user: HabitUser) {
  if (typeof window === "undefined") return
...
function getCurrentUser(): HabitUser | null {
...
function UserModal({
  ...
  useEffect(() => {
    if (mode !== "new" || !userId.trim()) {
      setIsIdTaken(false)
      return
    }
    setIsIdTaken(getUser(userId.trim()) !== null)
  }, [userId, mode])
    ...
    >
      ...
      >
        ...
        <button
          onClick={onClose}
          aria-label="Close"
          className="absolute right-5 top-5 z-[80] rounded-full p-1.5 text-muted-foreground transition-colors hover:bg-white/10 hover:text-foreground"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        >
          ...
          <div className="mt-6 space-y-4">
            {mode === "new" && (
              <Field label="Username" value={username} onChange={setUsername} placeholder="e.g. Alex Rivera" />
            )}
            <Field
              label="User ID"
              value={userId}
            ...
            {(error || isIdTaken) && (
              ...
              >
                ...
                >
                  ...
                  {isIdTaken ? "That User ID is already taken. Try another." : error}
                </motion.span>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <Magnetic strength={0.2} className="mt-6">
            <button
              ...
              disabled={isIdTaken}
              className={`w-full rounded-full bg-gradient-to-r from-[#34d399] to-[#0d9488] px-6 py-3.5 text-sm font-medium text-white shadow-[0_10px_40px_-12px_rgba(13,148,136,0.8)] ${isIdTaken ? 'opacity-50 cursor-not-allowed select-none' : 'active:scale-[0.98]'}`}
            >
              {isIdTaken ? "ID Unavailable" : "Continue"}
            </button>
          </Magnetic>
        </motion.div>
      </motion.div>
